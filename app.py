...
def detect_order_info_provided(text):
    ...
    if re.search(r'\d+\s+\w+\s+(st|street|ave|avenue|blvd|road|rd|dr|lane)',text,re.IGNORECASE):
        return True
    if len(text.split())>=2:  # input length check
        return True
    return False
...
# -----------------------------
    if detect_greeting(user_input):
        ...
        stop_words={"a","an","the","is","are","can","do","i","my","me","you","your"}

        def keyword_score(user,faq):
            user_words=set(re.findall(r"\b\w+\b",normalize(user)))-stop_words
            faq_words=set(re.findall(r"\b\w+\b",normalize(faq)))-stop_words
            if not user_words:
                return 0
            matched=user_words & faq_words
            return len(matched)/len(user_words)
        ...
        for item in faq_data:
            ...
            score=keyword_score(user_input,question)
            if score>best_score:
                best_score=score
                matched_faq=item
        ...
        if best_score>=THRESHOLD:
            ...
            # FAQ ID 7: show answer, then wait for order info
            if matched_faq["id"]==7:
                response=clean_response(matched_faq["answer"])
                st.session_state.waiting_for_tracking=True
                st.session_state.tracking_faq_id=7

            # FAQ ID 8: show exact answer from JSON
            elif matched_faq["id"]==8:
